...
class PemilikWisataMiddleware
{
    ...
    {
        // Cek apakah pengguna sudah login dengan guard 'pemilik_wisata'
        if (!Auth::guard('pemilik_wisata')->check()) {
            // Jika tidak, redirect ke halaman login dan kirimkan pesan error
            return redirect()->route('login')->with('error', 'Anda bukan pemilik wisata, silakan login sebagai pemilik.');
        }
